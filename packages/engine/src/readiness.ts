...
/** Actual body discomfort areas — excludes scan identifiers (ending in _scan) */
...
function buildMediumRationale(user: UserModel): string {
  ...
  if (hasScan) {
    ...
    return `Your ${scanType.replace('_', ' ')} scan came in recently — neutral readiness while your body adjusts.`;
  }
  if (skips > 0) {
...
function buildLowRationale(user: UserModel, actualDiscomfort: string[]): string {
  ...
  if (actualDiscomfort.length > 0 && sleep < 6.5) {
    const area = actualDiscomfort[0] ?? 'an area';
    return `Sleep has been short and ${area.replace('_', ' ')} is flagged — tonight is about protecting tomorrow.`;
  }
  if (actualDiscomfort.length > 0) {
    const area = actualDiscomfort[0] ?? 'an area';
    return `${area.replace('_', ' ')} is flagged — keeping today light protects the week ahead.`;
  }
