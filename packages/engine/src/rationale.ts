 ...
 *
 * BUG FIX: {days_phrase} is properly pluralized.
 * - 1 day → "1 day" (not "a day" to avoid "a day days" concatenation)
 * - N days → "N days"
 */
import type { CatalogCard, UserModel } from './types.ts';

interface TemplateVars {
  days_phrase: string; // "1 day" or "3 days" — never "a day"
...
/** Format a day count with correct singular/plural — e.g. "1 day" or "3 days" */
...
function buildVars(card: CatalogCard, user: UserModel): TemplateVars {
  ...
  const daysSinceLastSession = lastSessionHours / 24;
  const daysPhrase = formatDays(daysSinceLastSession);

  const areas = constraints_json.discomfort_areas.filter(
    (a) => !a.endsWith('_scan') && !a.startsWith('scan_'),
  );
  const areaText = areas.length > 0 ? (areas[0] ?? 'an area').replace(/_/g, ' ') : 'an area';
  ...
  if (areas.length > 0 && sleep < 6.5) {
    ...
    reasonShort = `${areaText} flagged and sleep thin`;
    reasonFull = `Your ${areaText} is flagged and sleep has been short — averaging ${sleep.toFixed(1)} hours`;
    ...
    reasonFull = `Sleep has been running short — averaging ${sleep.toFixed(1)} hours`;
  } else if (skips >= 3) {
  ...
  if (sessions >= 3) {
    ...
    weekObs = `You've logged 1 session this week`;
  } else {
...
function fillTemplate(template: string, vars: TemplateVars): string {
  return template
    .replace(/\{days_phrase\}/g, vars.days_phrase)
