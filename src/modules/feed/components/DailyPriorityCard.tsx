 ...
 * - Shuffle button (pill-shaped) at top-right corner with 3-min cooldown
 */
...
export function DailyPriorityCard({
  ...
  const cooldownMinutes = Math.ceil(shuffleCooldownSeconds / 60);
  ...
  return (
    <View style={[styles.card, { borderLeftColor: domainColor, borderLeftWidth: 4 }]}>
      ...
      <View style={styles.header}>
        ...
        {/* Shuffle button with 3-min cooldown display */}
        <Pressable
          ...
          accessibilityLabel={
            shuffleEnabled
              ...
              : `Shuffle available in ${cooldownMinutes} min`
          }
        >
          <Text style={[styles.shuffleText, !shuffleEnabled && styles.shuffleTextDisabled]}>
            {shuffleEnabled ? '⇄ Shuffle' : `⇄ ${cooldownMinutes}m`}
          </Text>
        </Pressable>
      </View>
      ...
      {/* Rationale — one sentence, muted per spec §11.1 */}
      ...
      {/* Why button — subtle, 32x32pt, 60% opacity per spec §12.2 */}
      <Pressable
